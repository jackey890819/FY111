...
namespace FY111.Controllers.API
{
    ...
    public class OccdisasterController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDisaster()
        {
            ...
            var data = await _context.ClassLittleunits.Include(x => x.Occdisasters)
                .Select(x => new { 
                    little_unit_code = x.Code, 
                    disasters = x.Occdisasters.Select(x => new {
                        code = x.Code,
                        content = x.Content
                    }).ToList() 
                }).ToListAsync();
