...
namespace FY111.Controllers.API
{
    ...
    public class OperationController : ControllerBase
    {
        ...
        {
            if (!_signInManager.IsSignedIn(User)) return BadRequest(new { errors = "Record failed" });
        ...
        {
            if (!_signInManager.IsSignedIn(User)) return BadRequest(new { errors = "Get record failed" });
            int operationId = _context.OperationUnitLogs.OrderBy(x => x.Id)
